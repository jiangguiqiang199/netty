 ...
 */
@Slf4j
public class MyPoolableObjectFactoryTest {
    ...
    public static void main(String[] args) {
        ...
        log.info("对象池初始化完成，活跃对象：{},空闲对象：{}",pool.getNumActive(),pool.getNumIdle());
        for (int i = 0; i < 10; i++) {
            new Thread(new Runnable() {
                public void run() {
                    try {
                        Resource resource = pool.borrowObject();// 注意，如果对象池没有空余的对象，那么这里会block，可以设置block的超时时间
                        log.info("获取对象前，对象池信息，活跃对象：{},空闲对象：{}",pool.getNumActive(),pool.getNumIdle());
                        log.info("获取对象:{}",resource);
                        Thread.sleep(1000);
                        log.info("获取对象后，归还对象前，对象池信息，活跃对象：{},空闲对象：{}",pool.getNumActive(),pool.getNumIdle());
                        pool.returnObject(resource);// 申请的资源用完了记得归还，不然其他人要申请时可能就没有资源用了
                        log.info("归还对象:{}",resource);
                        log.info("归还对象后，对象池信息，活跃对象：{},空闲对象：{}",pool.getNumActive(),pool.getNumIdle());
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
